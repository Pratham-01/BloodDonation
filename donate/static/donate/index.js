document.addEventListener('DOMContentLoaded', ()=>{
    ...
    if(document.querySelector('#submit2') !== null){
        document.querySelector('#submit2').addEventListener('click', ()=>{
            ...
            .then(response => {
                ...
                document.querySelector('#unitr').innerHTML = '';
    ...
    window.onresize = () => campScroll(x);
})

function campScroll(x) {
    if (x.matches) { // If media query matches
        var camps = document.getElementsByClassName('camps');
        ...
        var camps = document.getElementsByClassName('camps');
        for (var camp of camps) {
            camp.style = "display:flex; flex-flow: row wrap; justify-content: space-between; padding-right: 4%;";
        }
    }
}
...
function loadPage(city){
    ...
    fetch(`/filter/${city}`)
        ...
        .then(response => {
            ...
            campScroll(window.matchMedia("(max-width: 700px)"));
